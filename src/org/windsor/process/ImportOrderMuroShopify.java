 ...
 */
public class ImportOrderMuroShopify extends SvrProcess
{
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			else if (name.equals("C_OrderMuro_ID"))
...
//		log.fine("Found Location=" + actualizaBPL());*/
	...
	// ArrayList<String> recorredocerror = new ArrayList<String>();
		//	-- New Orders -----------------------------------------------------
	String slqoc  = "Select Documentno from T_NOTASHOPIFYMURO where processed<>'Y' and c_ordermuro_ID= "+p_order_id +  " Group by DocumentNo";
	...
	try
	{
		...
		while (rs.next())
		{
			String sqlcreados = "Select count(o.DocumentNO) as cuenta from T_NOTASHOPIFYMURO o  where o.processed<>'Y' "+
								" and o.poreference in (select b2.poreference from c_orderb2c b2 "+
								" where o.documentno=b2.documentomuro and b2.c_bpartner_ID=1001237) "
								+" and o.documentno='"+rs.getString("DocumentNo")+"'";
			log.fine("Script cuenta: "+sqlcreados);
			try
			{
				...
				if (rsc.next())
					if(rsc.getInt("cuenta")==0){
						...
						log.fine(rs.getString("DocumentNo"));
					}
	...
	int docnotok= 0;
		...
		{
			String recorre = "Select documentno, 1001237 as C_BPartner_ID,  1011338 as c_bpartner_location_ID,"+
							"	 COALESCE(M_product_ID, 0) M_product_ID, value, "+
							" replace(description,'/','-') description, "+
							"  COALESCE(QtyEntered,0) as Cantidad,  POReference,"+
							"  C_ORDERMURO_id, NOMBRESHOPIFY, DIRECCIONSHOPIFY  "+
							" from  T_NOTASHOPIFYMURO where  documentno='"+ ordenescompra.get(i)+"'";
				...
				while (rs.next()) //while crea ob2c 1 por 1
				{
					...
					if (ob2c==null)
					{
						//cabecera
						 ...
						 ob2c.setDateAcct(m_DateValue);
						 ob2c.set_CustomColumn("BPartnerValue", "76281810");
						 ob2c.set_CustomColumn("Address1", "Catalogo Digital");
...
//								ob2c.set_CustomColumn("numproductos", numproductosm);
					...
					if (ob2c!=null)
					{
						...
						obl.setProductValue(rs.getString("Value"));
						obl.save();
				...
				if ( errorp +errorbp +  errorbpl +  erroruser + errorst==0 || errorp==5) {
					...
					try {
						...
						if(rsvl.next()) {
							if(rsvl.getInt("cuenta")>0) {
								...
								String sqlvlok= "Select count(*) cuenta from C_OrderB2CLine where  C_OrderB2C_ID="+ob2c.getC_OrderB2C_ID();
								try {
											...
											try {
												...
												while (rslines.next()) {
													...
													if (contador<=25) {
														...
														{
															line.setQtyEntered( new BigDecimal (0));
													...
													if (contador==26) {
														if(completar) {
															String sqlNumProductosOrden = "SELECT COUNT(DISTINCT(M_Product_ID)) FROM C_OrderLine WHERE QtyEntered=0 C_Order_ID="+order.getC_Order_ID();
														...
														//	if (new BigDecimal(DB.getSQLValue(get_TrxName(), sqlNumProductosOrden)).compareTo(new BigDecimal(DB.getSQLValue(get_TrxName(), sqlNumProductos)))==0 || new BigDecimal(DB.getSQLValue(get_TrxName(), sqlUnidadesMuroOrden)).compareTo(new BigDecimal(DB.getSQLValue(get_TrxName(), sqlUnidadesMuro)))==0) {
															int qty0 = 0; //Integer.parseInt(DB.getSQLValueString(null, "SELECT COUNT(*) FROM C_OrderLine WHERE QtyEntered=0 and C_Order_ID= "+order.getC_Order_ID()															 ));
															qty0=order.getLines("and QtyEntered=0", "C_Order_ID").length;
															if(qty0==0)
															{
		...
		sql = new StringBuffer ("update		T_NOTASHOPIFYMURO  set processed='Y' where c_ordermuro_ID="+p_order_id);
	no = DB.executeUpdate(sql.toString(), get_TrxName());
	...
	{ 
		 ...
		 * 
		 * */
		sql = new StringBuffer ("UPDATE T_NOTASHOPIFYMURO o "
				  + "SET M_Product_ID=(SELECT MAX(M_Product_ID) FROM M_Product p"
				  + " WHERE trim(o.Value)=trim(p.Value) AND 1000000=p.AD_Client_ID) "
